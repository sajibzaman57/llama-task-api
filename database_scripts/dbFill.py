...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(
            argv, "hu:p:n:t:", ["url=", "port=", "users=", "tasks="])
    ...
    for i in range(userCount):
        ...
        params = urllib.parse.urlencode(
            {'name': firstNames[x] + " " + lastNames[y], 'email': firstNames[x] + "@" + lastNames[y] + ".com"})
    ...
    for i in range(taskCount):
        ...
        deadline = (mktime(date.today().timetuple()) +
                    randint(86400, 864000)) * 1000
        description = "It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as opposed to using 'Content here, content here', making it look like readable English."
        params = urllib.parse.urlencode({'name': choice(taskNames), 'deadline': deadline, 'assignedUserName': assignedUserName,
                                        'assignedUser': assignedUserID, 'completed': str(completed).lower(), 'description': description})
        ...
        if assigned and not completed:
            ...
            assignedUserTasks = [str(x).replace('[', '').replace(']', '').replace(
                "'", '').replace('"', '') for x in assignedUserTasks]
            assignedUserTasks.append(taskID)

            # PUT in the user
            params = urllib.parse.urlencode({'_id': assignedUserID, 'name': assignedUserName, 'email': assignedUserEmail,
                                            'dateCreated': assignedUserDate, 'pendingTasks': assignedUserTasks}, True)
    ...
    # Exit gracefully
    conn.close()
    print(str(userCount)+" users and "+str(taskCount) +
          " tasks added at "+baseurl+":"+str(port))
